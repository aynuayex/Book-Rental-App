...
import { Book } from "./DashboardTable";
...
const Books = () => {
  ...
  const handleChange = async (id: string, checked: boolean) => {
    try {
      ...
      const response = await axiosPrivate.post(`/books/${id}`, { rented, approved: checked });
  ...
  const columns = useMemo<GridColDef<(typeof books)[number]>[]>(() => [
    ...
    },
    {
      ...
      renderCell: (params: any) => {
        ...
        const checked = checkedState[id] !== undefined ? checkedState[id] : isApproved;
